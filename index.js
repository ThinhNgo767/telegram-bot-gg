...
bot.onText(/\/add (.+)/, (msg, match) => {
  ...
  if (!match[1].includes(",")) {
    bot.sendMessage(
      ...
      "Vui lòng nhập đúng định dạng." +
        "\n\n" +
        "Ví dụ:\n```\n/add name,compay,login,pass,code\n```",
      {
        parse_mode: "Markdown",
      }
    );
  ...
  url.searchParams.append("Company", values[1]);
  url.searchParams.append("Login", values[2]);
  url.searchParams.append("Pass", values[3]);
  url.searchParams.append("Code", values[4]);
